...
# =========================== CODE REVIEW REQUEST ===========================
# Review focus (1/4): 토큰 예산/트렁케이션 전략
# 1) Few-shot을 먼저 자르고, 그래도 부족하면 제시문을 자르는 순서가 맞는지
# 2) FEWSHOT_RATIO=0.6 고정이 적절한지, 혹은 문제 길이에 따라 동적으로 바꾸는 게 맞는지
# 3) 토큰 추정(//4) 기반 truncation이 실제 llama.cpp 토크나이저와 얼마나 어긋날 수 있는지,
#    리스크를 줄이려면 뭘 바꾸면 좋은지
# ===========================================================================
...
# =========================
# =========================== CODE REVIEW REQUEST ===========================
# Review focus (2/4): 프롬프트(시스템 메시지) 설계
# 1) Answer-only 강제 프롬프트가 실제로 정확도를 올리는지 /
#    혹은 reasoning 억제로 오히려 손해가 되는지
# 2) 예시 3개를 그대로 넣는 구조가 좋은지, 예시 길이/형식을 더 표준화해야 하는지
# 3) 프롬프트 엔지니어링 개선한다면 어떤 게 있을지
# ===========================================================================
...
# =========================== CODE REVIEW REQUEST ===========================
# Review focus (3/4): 재시도(복구) 정책
# 1) 500/timeout/empty 응답 같은 케이스에서 지금 재시도 정책이 충분한지
# 2) 재시도 때 seed만 바꾸는 게 최선인지, temperature/top_p도 같이 바꾸는 게 나은지
# 3) 실패 유형별로 재시도 분기(서버 오류 vs 형식 오류)를 더 나눌 필요가 있는지
# ===========================================================================
...
# =========================== CODE REVIEW REQUEST ===========================
# Review focus (4/4): 앙상블 방식
# 1) 현재 앙상블 다양성이 충분한지(프롬프트 2종 × 루프 2)
# 2) 다수결 말고 confidence 기반(예: logprob/자기검증)이 더 가치가 있는지
# ===========================================================================
